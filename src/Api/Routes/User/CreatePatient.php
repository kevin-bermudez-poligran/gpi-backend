...
use GpiPoligran\Exceptions\Service as ServiceError;
    use GpiPoligran\Services\Users\CreateUser;
     ...
     final class CreatePatient extends SuperAdminRoute{
        ...
        private function inputIsValid(){
            ...
            $this->validator->required('identification_number')->integer();
            
            $result = $this->validator->validateSchema([
                'name' => $this->request->body->name,
                'identification_number'     => $this->request->body->identification_number
            ]);
        ...
        public function run(){
            try{              
                ...
                $createUserService = new CreateUser(
                    ...
                    '',
                    '',
                    ProfilesEnum::PATIENT,
                    intval($this->request->body->identification_number)
                );
                $createUserService->register();

                return $this->sendResponse( 201,'Patient created' );
            }
